...
def preparar_lags(df, lag=12):
    """Cria colunas de lags para modelos baseados em histórico."""
    df = df.copy()
...
def safe_forecast_list(forecast_list):
    """Garante que valores de previsão sejam finitos."""
    safe = []
...
class MercadoTrabalhoPredictor:
    ...
    def converter_data_robusta(self, df_cbo):
        """Converte datas no formato YYYYMM para datetime."""
        try:
            df_cbo = df_cbo.copy()
    ...
    def prever_mercado_streamlit(self, cbo_codigo, anos_futuros=[5,10,15,20]):
        ...
        # Se dados insuficientes, projeção constante
        if len(df_mensal) < 10:
        ...
        if melhores:
            ...
            st.line_chart(pd.DataFrame({
                "Histórico": dados_melhor['historico'],
                "Previsão": [np.nan]*(len(dados_melhor['historico'])-1) + [dados_melhor['previsoes'][0]]*len(dados_melhor['previsoes'])
            }))

        else:
            st.warning("Nenhum modelo gerou resultados válidos.")

        # Tendência de saldo de mercado
        if 'saldomovimentacao' in df_cbo.columns:
            ...
            if saldo_ano.shape[0] >= 2:
                media_saldo = float(saldo_ano.tail(min(12, len(saldo_ano))).mean())
            ...
            if not np.isfinite(media_saldo): media_saldo = 0.0
            if not np.isfinite(crescimento_medio): crescimento_medio = 0.0

            st.subheader("Tendência de Mercado (Saldo)")
            if media_saldo > 100: status_atual = "ALTA DEMANDA"
            elif media_saldo > 0: status_atual = "CRESCIMENTO LEVE"
            elif media_saldo > -100: status_atual = "RETRAÇÃO LEVE"
            else: status_atual = "RETRAÇÃO FORTE"
            ...
            for a in anos_futuros:
                try:
                    ...
                    if not np.isfinite(proj) or abs(proj) > 1e12: proj = 0.0
                except:
                    proj = 0.0
...
base_path = Path.cwd()
filepath = base_path / "dados.parquet"
cbopath = base_path / "CBO.xlsx"
...
if entrada:
    ...
    if resultados.empty:
        ...
        cbo = resultados.loc[resultados['cbo_codigo'] == sel.split('[')[-1].replace(']','').strip(), 'cbo_codigo'].iloc[0]
